...
static constexpr const int xparts = 24;
static constexpr const int identity = width / xparts;
static constexpr const int origo_x = width / 2;
static constexpr const int origo_y = height / 2;
...
static void draw_board(SDL_Surface *window) {
    ...
    for (int i = origo_x; i <= width; i += identity)
        lineRGBA(window, i, 0, i, height, LTGRAY);

    for (int i = origo_y; i <= height; i += identity)
        lineRGBA(window, 0, i, width, i, LTGRAY);

    for (int i = origo_x; i >= 0; i -= identity) 
        lineRGBA(window, i, 0, i, height, LTGRAY);

    for (int i = origo_y; i >= 0; i -= identity) 
        lineRGBA(window, 0, i, width, i, LTGRAY);
...
bool plot_function(mash_e::container& f) {
    ...
    for (double i = -xparts - 1; i < xparts + 1; i += step) {
        ...
        lineRGBA(window, 
                 i1 * identity + origo_x, -f1 * identity + origo_y,
                 i2 * identity + origo_x, -f2 * identity + origo_y,
                 BLUE);
    }
