...
inline std::ostream& operator<<(std::ostream& stream, const mash_e::expression& expr) {
	expr.print(stream);
	return stream;
}
...
inline double riemann(const mash_e::container& f, double a, double b, double dx = 0.0001) {
	if (!f.is_valid() || near(a, b)) return 0.0;
	if (a > b) std::swap(a, b);
	mash_e::container fn = f->simplify();
